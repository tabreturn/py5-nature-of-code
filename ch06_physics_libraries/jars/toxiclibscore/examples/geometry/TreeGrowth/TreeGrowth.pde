...
class Branch {
  ...
  void update() {
    ...
    if (canBranch() && MathUtils.randomChance(0.08)) {
      ...
      float newLen=maxLen*MathUtils.random(0.5,0.85);
  ...
  // min 25% grown to full size
  // no more than 4 child branches
  boolean canBranch() {
    return depth<5 && len/maxLen>0.25 && children.size()<4;
  }
}
